...
export function getDemoDashboard(){if(!state)state=build();return state}
export function addDemoEvent(d:Detection[]){const s=getDemoDashboard();const mass=d.reduce((a,x)=>a+x.massKg,0);s.metrics.waste+=mass;s.metrics.yield=Math.round(s.metrics.waste*4);s.metrics.value=Math.round(s.metrics.waste*450);s.nodes=s.nodes.map((n,i)=>i===0?{...n,collectedToday:Math.round((n.collectedToday+mass)*10)/10,fill:Math.min(96,n.fill+Math.round(mass/3))}:n);s.detections=d;return s}
